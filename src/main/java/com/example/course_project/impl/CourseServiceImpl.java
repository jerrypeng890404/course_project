...
//		}
		...
		//防止輸入上課日不符合
		boolean courseDay = checkDay(req);
		if(!courseDay) {
			res.setMessage(CourseRtnCode.COURSEDAY_FAILED.getMessage());
			return res;
		}
		...
		if (req.getCourseStart() == null) {
			res.setMessage(CourseRtnCode.COURSESTART_REQUIRED.getMessage());
			return res;
		}
		if (req.getCourseEnd() == null) {
			res.setMessage(CourseRtnCode.COURSEEND_REQUIRED.getMessage());
			return res;
		}
	...
	// 私有防呆方法(防止輸入上課日不符合)
	private boolean checkDay(CourseRequest req) {
		...
		courseDay.add("THU");
		courseDay.add("FRI");
		return courseDay.contains(req.getCourseDay());
	}
	...
	public AddCourseResponse dropStudentCourse(String studentId, String courseCode) {
		...
		// 反之遍歷此學生所選的所有課程資訊
		for (StudentCourse studentInfo : studentCourseList) {
